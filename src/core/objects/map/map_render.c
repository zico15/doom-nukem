...
void __render_map_wall(t_map *this, SDL_Renderer *renderer, int iXShift, int iYShift)
{
    ...
    while (i < size)
    {
        ...
        SDL_RenderDrawLine(renderer,
                           ...
                           remap_x_screen(this, vEnd->x_position),
                           remap_y_screen(this, vEnd->y_position, renderer));
        i++;
    }
}
...
void __render_map_wall_player(t_map *this, SDL_Renderer *renderer, int iXShift, int iYShift)
{
    ...
    for (int i = 0; i < 9; ++i)
    {
        SDL_RenderDrawPoint(renderer,
                            remap_x_screen(this, this->player.x_position) + this->direction[i][0],
                            (remap_y_screen(this, this->player.y_position, renderer) + this->direction[i][1]));
    }
}
...
void __render_auto_map_node(t_map *this, SDL_Renderer *renderer, int node_id)
{
    ...
    SDL_Rect RightRect = {
        remap_x_screen(this, node->right_box_left),
        remap_y_screen(this, node->right_box_top, renderer),
        remap_x_screen(this, node->right_box_right) - remap_x_screen(this, node->right_box_left) + 1,
        remap_y_screen(this, node->right_box_bottom, renderer) - remap_y_screen(this, node->right_box_top, renderer) + 1};

    SDL_Rect LeftRect = {
        remap_x_screen(this, node->left_box_left),
        remap_y_screen(this, node->left_box_top, renderer),
        remap_x_screen(this, node->left_box_right) - remap_x_screen(this, node->left_box_left) + 1,
        remap_y_screen(this, node->left_box_bottom, renderer) - remap_y_screen(this, node->left_box_top, renderer) + 1};
    ...
    SDL_RenderDrawLine(renderer,
                       remap_x_screen(this, node->x_partition),
                       remap_y_screen(this, node->y_partition, renderer),
                       remap_x_screen(this, node->x_partition + node->change_x_partition),
                       remap_y_screen(this, node->y_partition + node->change_y_partition, renderer));
    // printf("x_partition: %i y_partition: %i\n", node->change_x_partition, node->change_y_partition);
}

bool is_on_left_side(t_map *map, int XPosition, int YPosition, int iNodeID)
{
