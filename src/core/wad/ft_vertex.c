...
static t_vertex *read_vertex_data(const uint8_t *pWADData, int offset)
{
    ...
    vertex = ft_calloc(sizeof(t_vertex));
...
void normalize_vertex(t_map *map)
{
    ...
    while (++i < size)
    {
        ...
        if (map->y_min > ((t_vertex *)(node[i]).value)->y_position)
            map->y_min = ((t_vertex *)(node[i]).value)->y_position;
        if (map->x_max < ((t_vertex *)(node[i]).value)->x_position)
            map->x_max = ((t_vertex *)(node[i]).value)->x_position;
        if (map->y_max < ((t_vertex *)(node[i]).value)->y_position)
            map->y_max = ((t_vertex *)(node[i]).value)->y_position;
    }
}
...
void read_map_vertex(t_wadd_data *wad_data, t_map *map)
{
    ...
    count_vertex = wad_data->directory[index].size / sizeof(t_vertex);
    i = 0;
    while (i < count_vertex)
    {
        vertex = (t_vertex *)array(map->vertexes)->add(read_vertex_data(wad_data->data, wad_data->directory[index].offset + i * sizeof(t_vertex)))->value;
        i++;
